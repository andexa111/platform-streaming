...
export default function AdsPage() {
  ...
  const uploadFileForSlot = async (slotId: number, file: File) => {
    ...
    setSlots((prev) => prev.map((s) => (s.id === slotId ? { ...s, uploading: true } : s)));
  ...
  const handleTextChange = (slotId: number, value: string) => {
    setSlots((prev) => prev.map((s) => (s.id === slotId ? { ...s, name: value } : s)));
  };

  const toggleActive = (slotId: number) => {
    setSlots((prev) => prev.map((s) => (s.id === slotId ? { ...s, is_active: !s.is_active } : s)));
  };

  const clearSlot = (slotId: number) => {
    setSlots((prev) => prev.map((s) => (s.id === slotId ? { ...s, logo_url: "", preview: "" } : s)));
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <StatusModal isOpen={modal.open} type={modal.type} title={modal.title} message={modal.message} onClose={() => setModal((m) => ({ ...m, open: false }))} />

      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-white">Manajemen Iklan Partner</h1>
          <p className="text-neutral-400 mt-1 text-sm">
            Kelola logo sponsor/partner (seperti Netflix, Google, dll) yang tampil secara dinamis di homepage member. <span className="text-blue-500 font-semibold">{activeCount}/6</span> slot terisi &amp; aktif.
          </p>
        </div>
        <Button onClick={handleSave} disabled={saving} className="self-start sm:self-auto bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl h-12 px-6 gap-2 disabled:opacity-60 transition-all duration-200 shrink-0">
        ...
        {slots.map((slot, index) => (
          <div
            ...
            className={`group bg-white dark:bg-neutral-900 border rounded-2xl overflow-hidden transition-all duration-300 ${
              slot.is_active ? "border-neutral-200 dark:border-neutral-800 hover:border-blue-500/40 hover:shadow-lg hover:shadow-blue-500/5" : "border-neutral-200/50 dark:border-neutral-800/40 opacity-60 dark:opacity-55"
            }`}
          >
            {/* Slot Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200 dark:border-neutral-800/50 bg-neutral-50 dark:bg-neutral-950/40">
              ...
              <div className="flex items-center gap-3">
                ...
                {/* Active Toggle */}
                <Switch checked={slot.is_active} onChange={() => toggleActive(slot.id)} label={slot.is_active ? "Aktif" : "Nonaktif"} />
              </div>
            </div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs text-neutral-600 dark:text-neutral-400 font-bold uppercase tracking-wider">Nama Brand</label>
                <input
                  type="text"
              ...
              {/* Upload Area */}
              <div className="space-y-1">
                <label className="text-xs text-neutral-600 dark:text-neutral-400 font-bold uppercase tracking-wider">File Logo</label>
                <input
                  ref={(el) => {
                ...
                >
                  {slot.uploading ? (
                    ...
                    <>
                      <img src={slot.preview} alt={`Slot ${slot.id}`} className="max-h-[100px] max-w-full w-auto object-contain p-3" />
                      {/* Overlay on hover */}
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                        <span className="text-white text-xs font-bold bg-neutral-900 rounded-lg px-3 py-1.5 border border-neutral-700">Ganti Gambar</span>
                      </div>
                    </>
                  ) : (
                    <div className="flex flex-col items-center gap-2 py-6 px-4 text-center pointer-events-none">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">Klik / Tarik Gambar Logo</p>
                        <p className="text-[10px] text-neutral-500 mt-0.5">PNG, JPG, SVG (Maks. 100MB)</p>
                      </div>
                    </div>
                  )}
                </div>
